...
const ModalBlock = ({ lst, type, setType }) => {
  ...
  return (
    <div className={css.modalTrigger} onClick={() => setModal(!modal)}>
      {type}
      <span className={css.triangleBox}>
        <Image
          ...
          height={5}
          className={
            modal ? `${css.triangle} ${css.triangleActive}` : css.triangle
          }
        />
      </span>
      <motion.div
        initial={{ height: 0 }}
        animate={{ height: modal ? "auto" : 0, opacity: modal ? 1 : 0 }}
        ...
        {modal &&
          lst.map((e) => (
            <span
              ...
              className={
                type == e
                  ? `${css.modalItem} ${css.modalItemActive}`
                  : css.modalItem
              }
              onClick={() => setType(e)}
            >
